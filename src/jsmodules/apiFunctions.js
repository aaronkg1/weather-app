...
async function getCurrentWeather(location, coords) {
  //if coords are omitted, will get weather based on location name
  let requestUrl;
  if (!coords) {
...
async function getDetailedForecast(location, units) {
  ...
  try {
    if (!location) {
      //if location is omitted,  get coordinates of client location and use these to build a forecast
      const userLocation = await getUserLocation();
      locationInfo = await getCurrentWeather(false , userLocation);
    } else {
